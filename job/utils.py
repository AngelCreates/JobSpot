...
def send_new_job_mail_func(applicant_list, job):
    for applicant in applicant_list:
        ...
        from_email = 'officialJobSpot777@gmail.com'
        ...
        msg_plain = loader.render_to_string(
            'email/job-notification.txt', context)
        msg_html = loader.render_to_string(
            'email/job-notification.html', context)

        send_mail(subject, msg_plain, from_email, [
                  applicant.user.email], fail_silently=True, html_message=msg_html)
    return
